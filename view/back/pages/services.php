...
<div class="srv-table-wrap reveal">
    <table class="srv-table">
        <thead>
            <tr><th>Image</th><th>Titre</th><th>Catégorie</th><th>Prix</th><th>Disponibilité</th><th>Statut</th><th>Actions</th></tr>
        </thead>
        ...
        <tr class="srv-row"
            ...
            <td style="font-weight:800;color:#ee5828;"><?= number_format((float)($s['prix']??0),2,',',' ') ?> €</td>
            <td><span style="display:inline-block;padding:5px 12px;border-radius:99px;font-size:11px;font-weight:800;<?= dBadge((string)($s['disponibilite']??'')) ?>"><?= htmlspecialchars($s['disponibilite']??'') ?></span></td>
            <td><span style="display:inline-block;padding:5px 12px;border-radius:99px;font-size:11px;font-weight:800;<?= sBadge((string)($s['statut']??'')) ?>"><?= htmlspecialchars($s['statut']??'') ?></span></td>
            <td>
...
new Chart(document.getElementById('chartCat'), {
    type: 'bar',
    data: { labels: catLabels, datasets: [{ label:'Services', data: catData, backgroundColor: pal, borderRadius:8, borderSkipped:false }] },
    options: {
...
new Chart(document.getElementById('chartStatut'), {
    ...
    data:{
        ...
        datasets:[{ data:statuts, backgroundColor:['#4cd774','#ffd04d','#aaaaaa'], borderWidth:0, hoverOffset:8 }]
    },
    options:{
        responsive:true, maintainAspectRatio:false, cutout:'68%',
        plugins:{ legend:{ position:'bottom', labels:{color:legendColor, font:{size:11}, padding:14, boxWidth:12} } }
    }
});
...
(function(){
    ...
    function filter(){
        ...
        let n = 0;
        rows().forEach(r => {
            const ok = (q===''||(r.dataset.titre||'').includes(q)||(r.dataset.categorie||'').includes(q))
                    && (st===''||(r.dataset.statut||'')===st);
            r.style.display = ok?'':'none';
            if(ok) n++;
        });
        count.textContent = n+' service(s)';
    }
