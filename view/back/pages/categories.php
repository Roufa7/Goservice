...
// ── IA : Prédiction des prix par catégorie (Régression Linéaire - php-ai/php-ml) ──
$tousServices    = $serviceController->listServicesWithCategories();
$predictionsPrix = RecommendationService::predirePrixParCategorie($tousServices);
...
<!-- ════════════════════════════════════════════════════════
     IA — Prédiction des prix par catégorie
     Régression Linéaire (Least Squares) · php-ai/php-ml · Composer
     ════════════════════════════════════════════════════════ -->
<?php if (!empty($predictionsPrix)): ?>
<style>
.ia-pred-section{margin-bottom:28px;}
.ia-pred-head{display:flex;align-items:center;gap:12px;margin-bottom:8px;flex-wrap:wrap;}
.ia-pred-badge{background:linear-gradient(135deg,rgba(238,88,40,.15),rgba(185,122,255,.15));
...
.ia-pred-title{font-size:18px;font-weight:900;color:var(--text);margin:0;}
.ia-pred-algo{font-size:11px;color:var(--muted);margin-bottom:18px;display:flex;align-items:center;gap:6px;}
.ia-pred-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(210px,1fr));gap:15px;}
.ia-pred-card{background:var(--panel);border:1px solid var(--border);border-radius:16px;
              padding:17px;box-shadow:0 6px 20px rgba(7,20,34,.06);transition:transform .2s;}
.ia-pred-card:hover{transform:translateY(-3px);}
.ia-pred-cat{font-size:12px;font-weight:800;color:var(--text);margin-bottom:12px;display:flex;align-items:center;gap:7px;}
.ia-pred-row{display:flex;justify-content:space-between;align-items:center;
             padding:6px 0;border-bottom:1px solid var(--border);font-size:12px;}
.ia-pred-row:last-child{border-bottom:none;padding-bottom:0;}
.ia-pred-lbl{color:var(--muted);}
.ia-pred-val{font-weight:700;color:var(--text);}
.ia-pred-val.prix-pred{color:#ee5828;font-size:14px;font-weight:900;}
.tend-hausse{color:#4cd774;font-weight:700;}
.tend-baisse{color:#ff8b8b;font-weight:700;}
.tend-stable{color:#6ea8ff;font-weight:700;}
.conf-bonne{color:#4cd774;font-size:11px;}
.conf-moyenne{color:#ffd04d;font-size:11px;}
.conf-faible{color:#ff8b8b;font-size:11px;}
</style>
<section class="ia-pred-section reveal">
    <div class="ia-pred-head">
        <span class="ia-pred-badge">🤖 IA</span>
        <h2 class="ia-pred-title">Prédiction des prix par catégorie</h2>
    </div>
    <div class="ia-pred-algo">
        🧠 Algorithme <strong>Régression Linéaire (Least Squares)</strong> — bibliothèque <strong>php-ai/php-ml</strong> · Composer
    </div>
    <div class="ia-pred-grid">
        <?php foreach ($predictionsPrix as $cat => $pred):
            $icon = '';
            foreach ($tousServices as $svc) {
                if (($svc['nom_categorie'] ?? '') === $cat) {
                    $icon = htmlspecialchars($svc['icone_categorie'] ?? '📂');
                    break;
                }
            }
            $tendIcon = match($pred['tendance']) { 'hausse' => '↗️', 'baisse' => '↘️', default => '→' };
        ?>
        <div class="ia-pred-card">
            <div class="ia-pred-cat"><?= $icon ?> <?= htmlspecialchars($cat) ?></div>
            <div class="ia-pred-row">
                <span class="ia-pred-lbl">Prix moyen actuel</span>
                <span class="ia-pred-val"><?= number_format($pred['moyenne_actuelle'], 2, ',', '') ?> €</span>
            </div>
            <div class="ia-pred-row">
                <span class="ia-pred-lbl">Prix prédit</span>
                <span class="ia-pred-val prix-pred"><?= number_format($pred['prediction'], 2, ',', '') ?> €</span>
            </div>
            <div class="ia-pred-row">
                <span class="ia-pred-lbl">Tendance</span>
                <span class="tend-<?= $pred['tendance'] ?>"><?= $tendIcon ?> <?= ucfirst($pred['tendance']) ?></span>
            </div>
            <div class="ia-pred-row">
                <span class="ia-pred-lbl">Services analysés</span>
                <span class="ia-pred-val"><?= $pred['nb_services'] ?></span>
            </div>
            <div class="ia-pred-row">
                <span class="ia-pred-lbl">Confiance</span>
                <span class="conf-<?= $pred['confiance'] ?>">● <?= ucfirst($pred['confiance']) ?></span>
            </div>
        </div>
        <?php endforeach; ?>
    </div>
