...
function forumUrl(array $extra=[]): string {
    $allowed=['page','search','filter','sort']; $base=['page'=>'forum'];
...
function forumGetPostWithUserById(int $postId): ?array {
    ...
    try{
        ...
        $sql="SELECT p.*, $sharedCol u.nom, u.prenom
              FROM post p
...
/* ---- Actions POST ---- */
if($_SERVER['REQUEST_METHOD']==='POST'&&isset($_POST['delete_post'])){ $deleteId=(int)($_POST['post_id']??0); if($deleteId>0){ $postController->deletePost($deleteId); header('Location: '.forumUrl(['deleted'=>1])); exit; } }
if($_SERVER['REQUEST_METHOD']==='POST'&&isset($_POST['delete_comment'])){ $commentId=(int)($_POST['comment_id']??0); $postId=(int)($_POST['post_id']??0); $parentId=(int)($_POST['parent_id']??0); if($commentId>0){ if($parentId===0){ if(method_exists($commentController,'deleteRepliesByComment')) $commentController->deleteRepliesByComment($commentId); else{ $all=$commentController->listCommentsByPost($postId); if(is_array($all)) foreach($all as $c) if((int)($c['id_parent_commentaire']??0)===$commentId) $commentController->deleteComment((int)$c['id_commentaire']); } } $commentController->deleteComment($commentId); } header('Location: '.forumUrl(['open_post'=>$postId,'comment_deleted'=>1])); exit; }
if($_SERVER['REQUEST_METHOD']==='POST'&&isset($_POST['update_comment'])){ $commentId=(int)($_POST['comment_id']??0); $postId=(int)($_POST['post_id']??0); $newContent=trim($_POST['comment_content']??''); $emojiContent=trim($_POST['emoji_content']??''); if($commentId>0){ if($newContent==='') $errors['comment']='Le commentaire est obligatoire.'; elseif(getLettersCount($newContent)<5) $errors['comment']='Le commentaire doit contenir au moins 5 lettres.'; elseif(!isEmojiOnlyForum($emojiContent)) $errors['comment']='Le champ emoji accepte uniquement des emojis.'; $oldComment=getCommentByIdForum($commentId); $imagePath=$oldComment['image_commentaire']??null; if(empty($errors['comment'])&&!empty($_FILES['comment_image']['name'])){ $ni=uploadCommentImageFile($_FILES['comment_image'],$errors,$imagePath); if(empty($errors['comment'])&&$ni!==null) $imagePath=$ni; } if(empty($errors['comment'])) updateCommentForum($commentId,$newContent,$imagePath,$emojiContent); } header('Location: '.forumUrl(['open_post'=>$postId,'comment_updated'=>1])); exit; }
...
if($_SERVER['REQUEST_METHOD']==='POST'&&(isset($_POST['publish_post'])||isset($_POST['update_post']))){
    ...
    if(isset($_POST['publish_post'])){ $imagePath=null; $videoPath=null; if(!$hasErrors&&!empty($_FILES['image']['name'])){ $imagePath=uploadImageFile($_FILES['image'],$errors); if(!empty($errors['image'])) $hasErrors=true; } if(!$hasErrors&&!empty($_FILES['video']['name'])){ $videoPath=uploadVideoFile($_FILES['video'],$errors); if(!empty($errors['video'])) $hasErrors=true; } if(!$hasErrors){ $post=new Post(null,$old['titre'],$old['contenu'],$imagePath,$videoPath,$old['type_post'],$old['statut_post'],$currentUserId);$postController->addPost($post);}
    ...
    if(isset($_POST['update_post'])){ $editId=(int)($_POST['edit_id']??0); $editPost=$postController->getPostById($editId); if($editPost){ $isEditMode=true; $imagePath=$editPost['image']??null; $videoPath=$editPost['video']??null; if(!$hasErrors&&!empty($_FILES['image']['name'])){ $imagePath=uploadImageFile($_FILES['image'],$errors,$editPost['image']??null); if(!empty($errors['image'])) $hasErrors=true; } if(!$hasErrors&&!empty($_FILES['video']['name'])){ $videoPath=uploadVideoFile($_FILES['video'],$errors,$editPost['video']??null); if(!empty($errors['video'])) $hasErrors=true; } if(!$hasErrors){ $post=new Post($editId,$old['titre'],$old['contenu'],$imagePath,$videoPath,$old['type_post'],$old['statut_post'],1); $postController->updatePost($post); updatePostEmojiForum($editId,$old['emoji_post']); updatePostGifForum($editId,$gifPost); header('Location: '.forumUrl(['updated'=>1])); exit; } } }
}}
        ...
        <form method="GET" action="/GoService/view/front/index.php" class="forum-search-layout">
            ...
            <div class="forum-bottom-actions">
                <div class="forum-filter-row">
                    <?php foreach(['Tous','Question','Conseil','Discussion'] as $f): $active=($filter===$f)||($filter==='Tous'&&$f==='Tous'); $link='/GoService/view/front/index.php?page=forum&filter='.urlencode($f).'&search='.urlencode($search).'&sort='.urlencode($sort); ?>
        ...
        <div class="forum-feed">
            ...
            <div class="forum-posts-list">
                ...
                <?php foreach($posts as $post):
                    ...
                    $viewerPayload=['id'=>(int)($post['id_post']??0),'type'=>!empty($videoUrl)?'video':'image','src'=>!empty($videoUrl)?$videoUrl:$imageUrl,'title'=>$post['titre']??'','content'=>$post['contenu']??'','user'=>$fullname,'time'=>timeAgo($post['date_publication']??''),'date'=>dateOnly($post['date_publication']??''),'image'=>$imageUrl,'video'=>$videoUrl,'likes'=>$likeCount,'comments'=>$commentCount,'shares'=>$shareCount,'saves'=>(int)($saveCount??0),'is_liked'=>!empty($post['is_liked']),'is_saved'=>!empty($post['is_saved']),'comments_data'=>$post['comments']??[]];
                ?>
    ...
    <div class="media-viewer-content" id="videoViewerContent">
        <div class="media-viewer-video-layout">
            ...
            <div class="media-viewer-video-side">
                ...
                <div class="media-viewer-action" id="reelCommentsWrap" style="display:none;"><div class="viewer-side-icon">💬</div><span id="reelCommentsCount">0</span></div>
                <div class="media-viewer-action" id="reelSharesWrap" style="display:none;"><div class="viewer-side-icon">🔁</div><span id="reelSharesCount">0</span></div>
                <div class="media-viewer-action"><div class="viewer-side-icon">⋯</div></div>
            </div>
        </div>
    </div>
</div>
...
/* ============================================================ MEDIA VIEWER */
function hideAllViewerModes(){imageViewerContent.classList.remove('show');videoViewerContent.classList.remove('show');imageViewerMain.innerHTML='';videoViewerMain.innerHTML='';}
...
function openMediaViewer(data){
    ...
    if(vComments){const ch=formatViewerComments(Array.isArray(data.comments_data)?data.comments_data:[]);vComments.innerHTML=ch||'<div class="viewer-comment-item"><div class="mini-avatar">U</div><div class="viewer-comment-bubble">Aucun commentaire pour le moment.</div></div>';}
    if(data.type==='video'){const v=document.createElement('video');v.src=data.src;v.controls=true;v.autoplay=true;v.loop=true;v.playsInline=true;videoViewerMain.appendChild(v);setCountVisibility('reelLikesWrap',likes,'reelLikesCount');setCountVisibility('reelCommentsWrap',comments,'reelCommentsCount');setCountVisibility('reelSharesWrap',shares,'reelSharesCount');videoViewerContent.classList.add('show');}
    else{const img=document.createElement('img');img.src=data.src;img.alt=safeTitle;imageViewerMain.appendChild(img);document.getElementById('viewerUserAvatar').textContent=safeUser.charAt(0).toUpperCase();document.getElementById('viewerUserName').textContent=safeUser;document.getElementById('viewerPostTime').textContent=safeTime;document.getElementById('viewerPostTitle').textContent=safeTitle;document.getElementById('viewerPostContent').innerHTML=safeContent.replace(/\n/g,'<br>');setCountVisibility('viewerLikesWrap',likes,'viewerLikesCount');setCountVisibility('viewerCommentsWrap',comments,'viewerCommentsCount');setCountVisibility('viewerSharesWrap',shares,'viewerSharesCount');imageViewerContent.classList.add('show');}
...
function startViewerEditComment(commentId,postId,content,emoji){const pf=document.getElementById('viewerPostId'),ta=document.getElementById('viewerCommentContent'),ei=document.getElementById('viewerEmojiHidden'),mini=document.getElementById('viewerCommentMini');viewerSetEditMode(commentId);if(pf) pf.value=postId;document.getElementById('viewerParentId').value='';openViewerCommentBox();if(ta){ta.value=content||'';ta.placeholder='Modifier votre commentaire...';ta.focus();ta.setSelectionRange(ta.value.length,ta.value.length);}if(ei) ei.value=emoji||'';if(mini&&mini.querySelector('span')) mini.querySelector('span').textContent='Modifier le commentaire';document.querySelectorAll('.viewer-comment-dropdown').forEach(m=>m.classList.remove('show'));}
...
function formatViewerComments(comments){
    const rootComments=[],repliesByParent={},currentPostId=document.getElementById('viewerPostId')?Number(document.getElementById('viewerPostId').value||0):0;
    ...
    return rootComments.map(comment=>{
        const cid=Number(comment.id_commentaire||0),author=`${comment.prenom||''} ${comment.nom||''}`.trim()||'Utilisateur',aL=author.charAt(0).toUpperCase(),content=viewerEscapeHtml(comment.contenu_commentaire||''),rawContent=viewerEscapeJs(comment.contenu_commentaire||''),emoji=viewerEscapeHtml(comment.emoji_commentaire||''),rawEmoji=viewerEscapeJs(comment.emoji_commentaire||''),time=viewerEscapeHtml(comment.date_commentaire||''),image=comment.image_commentaire?`/GoService/${String(comment.image_commentaire).replace(/^\/+/,'')})`:'',safeAJ=viewerEscapeJs(author),menuId=`viewer-cmenu-${cid}`,replies=repliesByParent[cid]||[];
        const repliesHtml=replies.map(reply=>{const rid=Number(reply.id_commentaire||0),ra=`${reply.prenom||''} ${reply.nom||''}`.trim()||'Utilisateur',rL=ra.charAt(0).toUpperCase(),rc=viewerEscapeHtml(reply.contenu_commentaire||''),rrc=viewerEscapeJs(reply.contenu_commentaire||''),re=viewerEscapeHtml(reply.emoji_commentaire||''),rre=viewerEscapeJs(reply.emoji_commentaire||''),ri=reply.image_commentaire?`/GoService/${String(reply.image_commentaire).replace(/^\/+/,'')})`:'',sraJ=viewerEscapeJs(ra),rmId=`viewer-cmenu-reply-${rid}`;return `<div class="viewer-reply-item"><div class="mini-avatar">${rL}</div><div class="viewer-comment-content"><div class="viewer-reply-bubble viewer-comment-bubble"><div class="viewer-comment-head-row"><span class="viewer-comment-author">${viewerEscapeHtml(ra)}</span><div class="viewer-comment-menu-wrap"><button type="button" class="viewer-comment-menu-btn" onclick="toggleViewerCommentMenu('${rmId}')">⋯</button><div class="viewer-comment-dropdown" id="${rmId}"><button type="button" onclick="startViewerEditComment(${rid},${currentPostId},'${rrc}','${rre}')">✏️ Modifier</button><button type="button" onclick="viewerReportComment(${rid},${currentPostId})">🚩 Signaler</button><button type="button" class="danger" onclick="viewerDeleteComment(${rid},${currentPostId},${cid})">🗑 Supprimer</button></div></div></div>${rc?`<div>${rc}</div>`:''}${re?`<div style="margin-top:6px;">${re}</div>`:''}${ri?`<div style="margin-top:8px;"><img src="${ri}" style="max-width:180px;border-radius:10px;"></div>`:''}</div><div class="viewer-comment-meta"><span>${viewerEscapeHtml(reply.date_commentaire||'')}</span><button type="button" class="viewer-reply-btn" onclick="setViewerReplyTarget(${cid},'${sraJ}')">Répondre</button></div></div></div>`;}).join('');
        const img2=comment.image_commentaire?`/GoService/${String(comment.image_commentaire).replace(/^\/+/,'')})`:'';
        return `<div class="viewer-comment-item"><div class="mini-avatar">${aL}</div><div class="viewer-comment-content"><div class="viewer-comment-bubble"><div class="viewer-comment-head-row"><span class="viewer-comment-author">${viewerEscapeHtml(author)}</span><div class="viewer-comment-menu-wrap"><button type="button" class="viewer-comment-menu-btn" onclick="toggleViewerCommentMenu('${menuId}')">⋯</button><div class="viewer-comment-dropdown" id="${menuId}"><button type="button" onclick="startViewerEditComment(${cid},${currentPostId},'${rawContent}','${rawEmoji}')">✏️ Modifier</button><button type="button" onclick="viewerReportComment(${cid},${currentPostId})">🚩 Signaler</button><button type="button" class="danger" onclick="viewerDeleteComment(${cid},${currentPostId},0)">🗑 Supprimer</button></div></div></div>${content?`<div>${content}</div>`:''}${emoji?`<div style="margin-top:6px;">${emoji}</div>`:''}${img2?`<div style="margin-top:8px;"><img src="${img2}" style="max-width:220px;border-radius:10px;"></div>`:''}</div><div class="viewer-comment-meta"><span>${time}</span><button type="button" class="viewer-reply-btn" onclick="setViewerReplyTarget(${cid},'${safeAJ}')">Répondre</button></div>${replies.length?`<div class="viewer-replies">${repliesHtml}</div>`:''}</div></div>`;
    }).join('');
}
