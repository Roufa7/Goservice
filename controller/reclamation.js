            ...
            .then(data => {
                ...
                if (data.success && data.reclamations.length > 0) {
                    data.reclamations.forEach(rec => {
                        ...
                        if (rec.rating) {
                            ratingHtml = `
                                <div class="review-stars" style="margin: 10px 0; font-size: 18px;">
                                    ${'★'.repeat(rec.rating)}${'☆'.repeat(5 - rec.rating)}
                                    <span style="font-size: 0.7em; color: #666; margin-left: 5px; vertical-align: middle;">(${rec.rating}/5)</span>
                                </div>
                                ${rec.avis_commentaire ? `<p style="font-style: italic; font-size: 0.9em; color: #555; margin-bottom: 10px; border-left: 3px solid #ffd700; padding-left: 10px;">"${escapeHtml(rec.avis_commentaire)}"</p>` : ''}
                            `;
                        } else {
                            ratingHtml = `<p style="font-size: 0.8em; color: #999; margin: 10px 0;">Aucun avis pour le moment.</p>`;
                        }

                        let reponseHtml = '';
                        if (rec.reponse_content) {
                            reponseHtml = `
                                <div class="reponse-box" style="margin: 10px 0; background: #f0f7ff; padding: 12px; border-radius: 8px; border-left: 4px solid #007bff;">
                                    <span style="font-weight: bold; font-size: 0.8em; color: #007bff; display: block; margin-bottom: 5px;">RÉPONSE DU SUPPORT :</span>
                                    <p style="font-size: 0.9em; color: #333; margin: 0;">${escapeHtml(rec.reponse_content)}</p>
                                </div>
                            `;
                        }
